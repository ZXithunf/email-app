...
export async function testFirestoreConnection(): Promise<boolean> {
  try {
    ...
    if (error instanceof Error && error.message.includes('the client is offline')) {
      console.warn('Firebase client is offline. Verify configuration and network.');
    } else {
      // Permission denied or missing doc is normal for test doc when rules exist
      console.info('Firestore connection tested successfully.');
    }
    return true;
  }
}
...
testFirestoreConnection().catch((err) => {
  console.warn('Firestore connection initial test ping notice:', err);
});
